...
export function AdminNavigation({ adminName, brand, panel, site }: { adminName: string; brand: string; panel: string; site: string }) {
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-40 bg-black/30 lg:hidden" onClick={() => setOpen(false)}><aside id="admin-mobile-nav" onClick={(event) => event.stopPropagation()} className="h-full w-72 max-w-[85vw] overflow-y-auto bg-card p-5 shadow-xl"><div className="mb-6 flex items-start justify-between gap-3"><div><span className="block font-semibold">{panel}</span><span className="mt-1 block text-sm text-foreground-muted">{adminName}</span></div><button type="button" aria-label="Close navigation" onClick={() => setOpen(false)} className="min-h-11 min-w-11 rounded-lg p-2"><X className="h-5 w-5" /></button></div>{nav(true)}</aside></div>}
  </>;
}
