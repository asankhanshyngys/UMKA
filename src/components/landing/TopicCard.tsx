      ...
      <ModuleCarousel>
        {course.modules.map((module, index) => (
          <Link key={module.id} href={`/courses/${course.id}`} className="group w-[calc((100vw-68px)/2)] shrink-0 snap-start rounded-2xl border border-border bg-card p-4 transition hover:-translate-y-0.5 hover:border-accent/40 sm:w-[calc((100vw-112px)/2)] lg:w-[260px]">
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-x-2 gap-y-1 border-t border-border pt-3 text-sm">
              <span className="inline-flex items-center gap-1 whitespace-nowrap text-foreground-muted"><PlayCircle className="h-4 w-4" /> {module.videos.length} {t("videos")}</span>
              <PriceTag price={module.price} oldPrice={module.oldPrice} />
            </div>
          </Link>
        ))}
      </ModuleCarousel>
