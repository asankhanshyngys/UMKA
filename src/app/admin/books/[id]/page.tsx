...
export default async function EditBookPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div><p className="text-sm text-foreground-subtle">Digital shop</p><h1 className="mt-1 font-serif text-4xl">Edit book</h1><form action={updateBook.bind(null, book.id)} encType="multipart/form-data" className="mt-8 grid max-w-2xl gap-4 md:grid-cols-2">
    ...
    <label className="text-sm md:col-span-2">Replace cover image (JPG, PNG or WebP; 5 MB max)<input name="cover" type="file" accept="image/jpeg,image/png,image/webp" className="mt-1 block w-full text-sm" /></label>
    ...
    <p className="text-sm text-foreground-muted md:col-span-2">The existing PDF and cover remain in use until you upload replacements.</p>
