...
export default async function CoursesPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {courses.length === 0 ? <div className="rounded-2xl border border-dashed border-border p-10 text-center text-foreground-muted">Курсов ещё нет. Создайте первый курс выше.</div> : <div className="grid gap-4">{courses.map((course) => <article key={course.id} className="rounded-2xl border border-border bg-card p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-3"><h2 className="text-xl font-semibold">{course.title}</h2><span className="rounded-full bg-background px-2.5 py-1 text-xs text-foreground-muted">{statusLabel[course.status]}</span></div><p className="mt-2 max-w-2xl text-sm text-foreground-muted">{course.description}</p><div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-sm text-foreground-subtle"><PriceTag price={course.price} oldPrice={course.oldPrice} /><span>{course.instructor.name}</span><span>{course.modules.length} модулей</span></div></div><div className="flex flex-wrap gap-2"><Link href={`/admin/courses/${course.id}/content`} className="inline-flex items-center gap-2 rounded-lg bg-accent px-3 py-2 text-sm text-white"><Layers3 className="h-4 w-4" />Программа</Link><Link href={`/admin/courses/${course.id}`} className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm"><Pencil className="h-4 w-4" />Изменить</Link><form action={deleteCourse.bind(null, course.id)}><AdminSubmitButton pendingLabel="Удаляю…" className="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-red-700 hover:bg-red-50"><Trash2 className="h-4 w-4" />Удалить</AdminSubmitButton></form></div></div></article>)}</div>}
    </div>
  );
}
