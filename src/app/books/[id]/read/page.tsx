...
export default async function ReadBookPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="min-h-screen bg-background"><Header /><main className="main-with-mobile-nav px-4 py-8 sm:px-6 lg:px-12 xl:px-20"><p className="text-sm text-foreground-subtle"><Link href="/library">{t("myLibrary")}</Link></p><h1 className="mt-1 font-serif text-3xl text-foreground">{book.title}</h1><BookReader bookId={book.id} title={book.title} hasAccess /></main></div>;
}
