...
export default async function BookPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="min-h-screen bg-background"><Header /><main className="main-with-mobile-nav px-4 py-14 sm:px-6 lg:px-12 xl:px-20"><div className="grid gap-10 md:grid-cols-[0.8fr_1.2fr]"><div className="relative aspect-[4/5] overflow-hidden rounded-3xl bg-card">{getBookCoverUrl(book) && <Image src={getBookCoverUrl(book)!} alt={t("coverAlt", { title: book.title })} fill priority className="object-cover" sizes="(max-width: 768px) 100vw, 40vw" />}</div><div className="flex flex-col justify-center"><p className="text-xs font-medium uppercase tracking-label text-foreground-subtle">{t("digitalBook", { author: book.author })}</p><h1 className="mt-4 font-serif text-4xl leading-tight text-foreground sm:text-5xl">{book.title}</h1><p className="mt-6 max-w-xl text-base leading-relaxed text-foreground-muted">{book.description}</p><PriceTag price={book.price} oldPrice={book.oldPrice} className="mt-8 font-serif text-3xl" /><p className="mt-4 text-sm text-foreground-muted">{t("permanentAccess")}</p></div></div><BookReader bookId={book.id} title={book.title} hasAccess={hasAccess} /></main></div>;
}
