...
export default function VerifyEmailPage() {
  ...
  return <main className="flex min-h-screen items-center justify-center bg-background px-6"><section className="w-full max-w-md space-y-5 rounded-2xl border border-border bg-card p-8 shadow-sm"><p className="text-sm text-foreground-subtle">UMKA</p><h1 className="font-serif text-3xl text-foreground">Email verification</h1><p role="status" className="text-foreground-muted">{message}</p><Link href="/login" className="inline-block text-sm text-accent underline">Go to sign in</Link></section></main>;
}
